...
/// On Windows a bundled runtime ships as `node.exe`, but `launcher.toml` names
/// the command `node`. If the extension-less program isn't present and a `.exe`
/// sibling is, prefer that so the spawn finds the executable. No-op elsewhere.
#[cfg(windows)]
