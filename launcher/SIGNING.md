# Signing & notarizing the ATEM Fleet Admin desktop app (optional)

**You do not need any of this to use ATEM Fleet Admin.** By default the `.dmg` is
...
certificate (the cert type for distributing apps _outside_ the App Store).
...
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `APPLE_CERTIFICATE`          | Your _Developer ID Application_ certificate exported from **Keychain Access** as a `.p12`, then base64-encoded: `base64 -i cert.p12 \| pbcopy`. Paste the result.                                 |
...
`release-desktop.yml` (on a `v*` tag or manual run):
